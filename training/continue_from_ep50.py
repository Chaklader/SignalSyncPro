...
def continue_training():
    ...
    for episode_idx in tqdm(range(num_episodes), desc="Training"):
        ...
        print(f"  Reward: {avg_reward:.4f} | Loss: {loss_str} | Steps: {step_count} | Epsilon: {agent.epsilon:.3f}")
        print(f"  Avg Wait: {final_metrics['avg_waiting_time']:.2f}s | Sync Rate: {final_metrics['sync_success_rate']:.2%}")
        print(f"  Car: {final_metrics['waiting_time_car']:.2f}s | Bike: {final_metrics['waiting_time_bicycle']:.2f}s | Bus: {final_metrics['waiting_time_bus']:.2f}s")
